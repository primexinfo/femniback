...
// If guest checkout is Deactivated then display pop up form with proper error message
    ...
    public function cashondelivery(Request $request)
    {
        if($request->pass_check) {
            ...
            if(count($users) == 0) {
                if ($request->personal_pass == $request->personal_confirm  && $request->personal_email !=null){
                    $user = new User;
    ...
    public function amarpay(Request $request){

        $input = $request->all();
        \Illuminate\Support\Facades\Session::put('amarpay',$input);
        ...
        $paymentAmount = round($request->total / $curr->value, 2) + $request->shipping_cost + $request->packing_cost;
        $customer_email = $request->email;
        $customer_name = $request->name;
        $customer_phone = $request->phone;
        return view('load.amarpay',compact('paymentAmount','customer_email','customer_name','customer_phone'));
    }
